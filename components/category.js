...
const Category = ({products, title}) => {
  return (
    <div className='mt-10 bg-inherit relative '>
        {/* header */}
        <div className='w-full relative h-10  flex items-center justify-center bg-inherit'>
            ...
            <p className='text-center relative h-auto w-auto m-0 leading-none px-3 bg-inherit z-100 text-xl font-semibold text-black/70 capitalize'>{title}</p>
        </div>
        <div className='absolute top-0 right-0 bottom-0 m-auto h-fit p-4 bg-slate-400/60 z-30 '>
        ...
        {/* Products */}
        <div className='gap-5 flex overflow-y-auto '>
            
            { products.map((item)=>(
             <Link href={`/product/${item._id}`}  key = {item.name}>   
            <div  className = 'relative product p-2   '>
                {/* hover effect */}
                ...
                {/* item content */}
                <div className=' h-[200px] w-[200px] md:h-[400px] md:w-[400px] relative ' >
                    <Image src={item.product} alt = {item.product} width= '100%' height = '105%' layout='responsive'/>
                </div>
                <div className='relative mt-4 w-[200px] md:w-[400px] overflow-hidden '>
                    <h2 className='font-semibold text-lg uppercase'>{item.name}</h2>
                    <p className='text-black/60 font-semibold max-lines w-full'>{item.description}</p>
                    <div className='flex space-x-4 mt-2'>
                        <p className='leading-none text-lg'>${item.discount ? (0.01 * item.discount * item.price): item.price} </p>
                    ...
                    <div className = 'flex justify-between items-center mt-3 px-3'>
                        {item.discount && (<div className='bg-green-500/40 p-1 border-r-4 w-fit '>
                            <p>You save <span className='font-semibold'>${item.price - (0.01 * item.discount * item.price)}</span></p>
                        </div>)}
                        {/* <div className = ''>
